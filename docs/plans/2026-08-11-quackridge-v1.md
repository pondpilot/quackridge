...
- [ ] Create the public `pondpilot/quackridge` GitHub repository with `main` as its default branch, Apache-2.0 licensing, an accurate description, and relevant DuckDB/PostgreSQL/PondPilot topics.
- [ ] Initialize the local repository with Go module path `github.com/pondpilot/quackridge`; preserve this plan under `docs-plans/`.
- [ ] Add the Apache-2.0 `LICENSE`, `README.md`, `CONTRIBUTING.md`, `SECURITY.md`, `.gitignore`, `.editorconfig`, and an experimental-status notice.
